...
export default async function handler(req, res) {
    try {
        ...
        if (cleanJobs > 0) {
            exec(`gcloud run jobs execute clean-video-worker --region=asia-southeast1 --project=onlook-main`, (err, stdout, stderr) => {
        ...
        if (mergeJobs > 0) {
            exec(`gcloud run jobs execute merge-video-worker --region=asia-southeast1 --project=onlook-main`, (err, stdout, stderr) => {
        ...
        if (uploadJobs > 0) {
            exec(`gcloud run jobs execute upload-video-worker --region=asia-southeast1 --project=onlook-main`, (err, stdout, stderr) => {
