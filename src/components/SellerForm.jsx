...
const SellerForm = ({ mode, onSubmit }) => {
    ...
    return (<div className="space-y-6">
            <ProductInfoBox productName={productName} setProductName={setProductName} link={link} setLink={setLink}/>

            {(mode === 'file-only' || mode === 'video-audio') && (<>
                    <FileUploader label="🎥 Video sản phẩm (.mp4)" accept="video/mp4" folder="videos" onUploadComplete={setVideoUrl}/>
                    <Player fileUrl={videoUrl} type="video"/>
                </>)}

            {(mode === 'video-audio' || mode === 'webcam-audio') && (<>
                    <FileUploader label="🎧 File âm thanh giới thiệu (.mp3)" accept="audio/mpeg" folder="audios" onUploadComplete={setAudioUrl}/>
